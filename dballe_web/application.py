...
class APIView(MethodView):
    ...
    def call_api(self, kwargs: Dict[str, Any]):
        try:
            ...
            app.logger.debug("API call %s %r result %r", self.__class__.__name__, kwargs, result)
            if not self.db_session.initialized:
                result["initializing"] = True
            result["time"] = time.time()
            return jsonify(result)
        except Exception as e:
            app.logger.error("API call error %s", e, exc_info=True)
            code = 500
    ...
    @property
    def db_session(self):
        return request.environ["dballeweb.db_session"]

    @classmethod
    def register(cls, name: str):
        app.add_url_rule(f"/api/1.0/{name}", view_func=cls.as_view(f"api1.0_{name}"))
...
class APIViewGET(APIView):
    ...
    """
    def get(self, **kwargs):
        app.logger.debug("API GET call %s %r", self.__class__.__name__, kwargs)
...
class APIViewPOST(APIView):
    ...
    def post(self, **kwargs):
        app.logger.debug("API POST call %s %r", self.__class__.__name__, kwargs)
